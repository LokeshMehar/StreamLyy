...
function NavbarRightContent({ mobile = false }: { mobile?: boolean }) {
  ...
  // Initialize userId and profilePic from localStorage on client-side
  useEffect(() => {
  ...
  // Fetch profile picture when fetchAgain changes
  useEffect(() => {
    if (!userId) return;

    axios
      .get(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/v1/upload/get-profile-pic/${userId}`)
      .then((res) => {
